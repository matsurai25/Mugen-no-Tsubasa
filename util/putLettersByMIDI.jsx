...
/*
 ...
 ## 用途
 * コンポジションごとに文字分けちゃったので、その配置

 ## 結果
 * MIDIに合わせてletter_{s}なコンポジションを配置
 * それぞれにタイムリマップを適応

 ## 必要
 * project内にletter_{s}なコンポジションが存在すること。なければCompDuplicateで増やす
 * vocalだけを抽出したMIDIが存在すること
 * "文字,音数"の歌詞CSVが存在すること
...
// set vars
var offset = range / effectSpeed;
var Lyrics = getLilycs();
var exp = "var range = "+range+"; var effectSpeed = "+effectSpeed+"; var currentTime; if(time>outPoint-(range/effectSpeed)){currentTime = (range*2)-(outPoint-time) * effectSpeed; }else if(time<inPoint+(range/effectSpeed)){currentTime = (time - inPoint) * effectSpeed; }else{currentTime = range; } currentTime;";
...

{
  ...
  function applyChannelToLayer(channel,layer,pb,totalNotesCount,notesSoFar,myComp)
  {
    ...
    for(var i = 0; i < channel.notes.length; i++)
    {
      ...
      if(vel && typeof(dur) != "undefined")
      {
        ...
        if(ConsecutiveNum == noteNum){
          if(typeof(Lyrics[lyricsCount].letter) != "undefined"){
            break
          }
          var target = myComp.layers.add(items[searchLetterIndex(Lyrics[lyricsCount].letter)])
          target.startTime = t - offset;
          //タイムリマップ適応
          target.timeRemapEnabled = true;
          target.timeRemap.removeKey(2);
          target.timeRemap.expression = exp;

          ConsecutiveNum = Number(ConsecutiveNum)+Number(Lyrics[lyricsCount].words);
  ...
  function applyMidiToLayer(midi,layer,totalNotesCount,myComp)
  {
    ...
    function apply(result)
    {
        ...
        var myComp = app.project.items.addComp("putLettesbyMIDI", 1920, 1080, 1.00, 600, 29.97)
        app.beginUndoGroup("putLettesbyMIDI");
